...
### Boxplot
p1 <- ggplot(Data)+
  geom_boxplot(aes(x = Samples, y = Percentage_of_mobile_larvae, color = Samples, fill = Samples), alpha = 0.65)+
  ...
  geom_point(aes(x = Samples, y = Percentage_of_mobile_larvae, color = Samples))+
  scale_color_manual("Samples", values = Colors, guide = "none")+
  stat_summary(aes(x = Samples, y = Percentage_of_mobile_larvae), fun = mean, colour = "black",  geom = "point", shape = 3, size = 2)+
...
ggsave(plot = p1, dpi = 1000, device = "jpeg", width = 7, height = 4, filename = "Results/Figure_1.jpeg")
